...
- User config: `XDG_CONFIG_HOME/tlm/config.toml` (`tlm/settings.py`); permissions / freelist: `XDG_CONFIG_HOME/tlm/permissions.toml` (`tlm/safety/permissions.py`). **Web in ask:** `web_enabled = true` via a browser backend (Lightpanda or Playwright) — set via `tlm init --wizard`, `tlm config` (TUI `w`), or **GUI** Web tab. Install: from git / GitHub URL until PyPI (`docs/install.md`).
- Canonical requirements: `requirements.txt` + `pyproject.toml` deps should stay aligned.
- **Versioning:** Use **`.devN`** for development (`0.2.0.dev1`, …) and **`bN`** for beta (`0.2.0b4`, …), per PEP 440. **CRITICAL:** ALWAYS bump the subversion (e.g., `0.4.0.dev1` → `0.4.0.dev2`) and add a corresponding entry to `CHANGELOG.md` at EVERY substantive step or task completion. Update `VERSION` (Dev line), `pyproject.toml`, `src/tlm/_version.py` (fallback), and the `requirements.txt` comment to stay aligned. Runtime `tlm --version` reads installed metadata (`pyproject.toml`).
