    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="field"):
                ...
                yield Select(
                    [("strict", "strict"), ("standard", "standard"), ("trusted", "trusted"), ("sandbox", "sandbox")],
                    value=self.settings.safety_profile,
                    id="set_safety"
                )

            yield Label("Memory", classes="section-title")
            yield Checkbox("Enable Memory", value=self.settings.memory_enabled, id="set_mem_enabled")
            ...
            yield Input(value=current_key, placeholder=f"API Key for {current_provider}", password=True, id="set_api_key")
