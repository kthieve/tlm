...
from tlm.providers.registry import REAL_PROVIDER_IDS, get_provider, list_provider_ids, normalize_provider_id
from tlm.settings import UserSettings, load_settings, save_settings, merged_api_key
from tlm.providers.openai_compat import DEFAULT_MODELS
from tlm.gui.tips import BUDGET_PICKS, DAILY_COST_ESTIMATE, FEATURE_TIPS, PROVIDER_SIGNUP_URLS, QUICKSTART_COMMANDS, USEFUL_COMMANDS
...
class OnboardingWizard:
    ...
    def _build_layout(self):
        ...
        self.step_label = tk.Label(self.header, text="Setup Wizard", font=self.base_font, fg=_FG_MUTED, bg=_BG_HEADER)
    ...
    def _create_provider_page(self):
        self._create_tip_sidebar(self.content_frame, "Choosing a Provider", 
        ...
        tk.Label(main, text="LLM Provider Setup", font=self.title_font, bg=_BG_PAGE).pack(anchor="w", pady=(0, 20))
        ...
        prov_cb = ttk.Combobox(form, textvariable=prov_var, values=list_provider_ids(), state="readonly", width=30)
        ...
        ttk.Checkbutton(form, text="Show", variable=show_var, command=toggle_key).grid(row=1, column=2, sticky="w")
        
        tk.Label(form, text="Default Model", bg=_BG_PAGE).grid(row=2, column=0, sticky="w", pady=5)
        model_var = tk.StringVar(value=self.settings.model or DEFAULT_MODELS.get(prov_var.get(), ""))
        model_ent = ttk.Entry(form, textvariable=model_var, width=45)
        model_ent.grid(row=2, column=1, sticky="w", padx=10, pady=5)
            ...
            def worker():
                try:
                    s = UserSettings(
                        ...
                        api_keys={prov_var.get(): key_var.get()},
                        model=model_ent.get()
                    )
                    ...
                    self.root.after(0, lambda: status_var.set(f"❌ Failed: {str(e)[:40]}..."))
                    self.root.after(0, lambda: status_lbl.configure(fg=_ERROR))
            
            threading.Thread(target=worker, daemon=True).start()

        def save_only():
            self.settings.provider = prov_var.get()
            self.settings.api_keys[self.settings.provider] = key_var.get()
            self.settings.model = model_ent.get()
            save_settings(self.settings)
            messagebox.showinfo("tlm", "Settings saved.")

        btn_fr = tk.Frame(main, bg=_BG_PAGE)
        btn_fr.pack(anchor="e", pady=10)
        ttk.Button(btn_fr, text="Test Connection", command=test_connection).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_fr, text="Save Settings", command=save_only).pack(side=tk.LEFT)

    def _create_features_page(self):
        self._create_tip_sidebar(self.content_frame, "Core Features", 
            FEATURE_TIPS["memory"] + "\n\n" + FEATURE_TIPS["web"])
        
        main = tk.Frame(self.content_frame, bg=_BG_PAGE)
        main.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        tk.Label(main, text="Enable Features", font=self.title_font, bg=_BG_PAGE).pack(anchor="w", pady=(0, 20))
        
        mem_var = tk.BooleanVar(value=self.settings.memory_enabled)
        ttk.Checkbutton(main, text="Enable Ready/Long-term Memory", variable=mem_var).pack(anchor="w", pady=5)
        
        upd_var = tk.BooleanVar(value=self.settings.check_for_updates)
        ttk.Checkbutton(main, text="Auto-check for updates (daily)", variable=upd_var).pack(anchor="w", pady=5)
        
        web_var = tk.BooleanVar(value=self.settings.web_enabled)
        ttk.Checkbutton(main, text="Enable Web Tools (Lightpanda)", variable=web_var).pack(anchor="w", pady=5)
        ...
        prof_cb = ttk.Combobox(main, textvariable=prof_var, values=["strict", "standard", "trusted", "sandbox"], state="readonly")
    ...
    def _create_guide_page(self):
        tk.Label(self.content_frame, text="🚀 You're Ready!", font=self.header_font, bg=_BG_PAGE).pack(pady=(0, 20))
        
        # Examples
        card = tk.Frame(self.content_frame, bg=_BG_CARD, padx=20, pady=20, highlightbackground=_FG_MUTED, highlightthickness=1)
        card.pack(fill=tk.BOTH, expand=True)
        
        tk.Label(card, text="Try these commands in your terminal:", font=self.title_font, bg=_BG_CARD).pack(anchor="w", pady=(0, 15))
        
        for title, cmd in QUICKSTART_COMMANDS:
            tk.Label(card, text=title, font=self.base_font, bg=_BG_CARD, fg=_FG_MUTED).pack(anchor="w")
    ...
    def _create_path_page(self):
        ...
        tk.Label(self.content_frame, text="📎 Final Step — PATH Setup", font=self.header_font, bg=_BG_PAGE).pack(pady=(0, 20))
        
        tk.Label(self.content_frame, text="To run 'tlm' from anywhere, ensure the bin directory is on your PATH.", bg=_BG_PAGE, wraplength=600).pack(pady=10)
        ...
        tk.Label(self.content_frame, text=f"Detected bin path: {bin_dir or 'unknown'}", font=self.tip_font, bg=_BG_PAGE).pack()

        tk.Label(self.content_frame, text="Recommended: Add to ~/.bashrc or ~/.zshrc", font=self.title_font, bg=_BG_PAGE).pack(pady=(20, 10))
        ...
        def add_to_rc(shell):
            ...
            try:
                with open(rc, "a") as f:
                    f.write(line)
                messagebox.showinfo("Success", f"Added to {rc}. Please restart your terminal or run 'source {rc}'.")
            except Exception as e:
        ...
        ttk.Button(btn_fr, text="Add to ~/.bashrc", command=lambda: add_to_rc("bash")).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_fr, text="Add to ~/.zshrc", command=lambda: add_to_rc("zsh")).pack(side=tk.LEFT, padx=10)
